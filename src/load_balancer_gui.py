...
class LoadBalancerGUI(QMainWindow):
    def __init__(self):
        ...
        # Processor States
        self.states_group = QGroupBox("Processor States")
        states_layout = QVBoxLayout()
        self.state_labels = {}
        self.states_group.setLayout(states_layout)
        control_layout.addWidget(self.states_group)
        
        # Initialize processor states based on current selection
        self.update_processor_states()
        ...
        self.processor_tasks = {i: [] for i in range(self.proc_spin.value())}  # Track tasks for each processor
    
    def update_processor_states(self):
        # Clear existing state labels
        for label in self.state_labels.values():
            label.setParent(None)
        self.state_labels.clear()
        
        # Create labels only for selected number of processors
        for i in range(self.proc_spin.value()):
            label = QLabel(f"Processor {i}: IDLE")
            self.state_labels[i] = label
            self.states_group.layout().addWidget(label)
    ...
    def update_process_settings(self):
        ...
        # Update processor states when number of processors changes
        self.update_processor_states()
        # Reset processor task tracking
        self.processor_tasks = {i: [] for i in range(self.proc_spin.value())}
    ...
    def start_simulation(self):
        ...
        self.log_text.clear()
        self.log_message("=== Dynamic Load Balancer Simulation Started ===")
        self.log_message("\nConfiguration:")
        self.log_message("=" * 50)
        self.log_message(f"Number of Processors: {self.proc_spin.value()}")
        
        # Log individual processor settings
        for i, settings in enumerate(self.process_settings):
            self.log_message(f"Processor {i}:")
            self.log_message(f"  - Processing Speed: {settings['speed'].value():.1f}x")
            self.log_message(f"  - Queue Size Limit: {settings['queue_size'].value()} tasks")
        
        self.log_message("\nTask Settings:")
        self.log_message("=" * 50)
        self.log_message(f"Total Tasks to Generate: {self.max_tasks}")
        self.log_message(f"Task Generation Probability: {self.prob_spin.value():.1f}")
        self.log_message(f"Task Priority Level: {self.priority_spin.value()}")
        self.log_message(f"Task Workload Range: {self.workload_min.value():.1f} - {self.workload_max.value():.1f}")
        
        self.log_message("\nLoad Balancing Settings:")
        self.log_message("=" * 50)
        self.log_message(f"Balance Threshold: {self.threshold_spin.value():.1f}")
        self.log_message(f"Balance Check Frequency: Every {self.freq_spin.value()} steps")
    ...
    def stop_simulation(self):
        ...
        if self.balancer:
            ...
            self.log_message("\nFinal Statistics:")
            self.log_message("-" * 30)
            self.log_message(f"Total Tasks Generated: {stats['total_tasks']}")
            self.log_message(f"Tasks Completed: {stats['completed_tasks']}")
            self.log_message(f"Load Balance Operations: {stats['load_balance_count']}")
            self.log_message(f"Task Migrations: {stats['total_migrations']}")
            self.log_message(f"Average Processing Time: {stats['avg_processing_time']:.2f}")
            self.log_message(f"Total Simulation Time: {stats['elapsed_time']:.1f}s")
            
            self.log_message("\nFinal Task Distribution:")
            self.log_message("-" * 30)
            for processor_id in range(self.proc_spin.value()):
                tasks = self.processor_tasks[processor_id]
                if tasks:
                    self.log_message(
                        f"Processor {processor_id}:"
                        f"\n  - Tasks Handled: {len(tasks)}"
                        f"\n  - Task IDs: {tasks}"
                    )
    ...
    def simulation_step(self):
        if self.balancer.total_tasks < self.max_tasks:
            if np.random.random() < self.prob_spin.value():
                ...
                if processor >= 0:
                    self.processor_tasks[processor].append(task.task_id)
                    ...
                    self.log_message(
                        ...
                        f"\n    - Priority: {priority}"
                        f"\n  Assignment:"
                        f"\n    - Assigned to: Processor {processor}"
                        f"\n    - Processor Queue: {self.processor_tasks[processor]}"
                        f"\n  System Status:"
                        f"\n    - Current Loads: {[f'{load:.2f}' for load in loads]}"
                    )
        ...
        if self.step % self.freq_spin.value() == 0:
            ...
            loads = self.balancer.get_processor_loads()
            self.log_message(
                f"\nLoad Balance Check (Step {self.step}):"
                f"\n  - Current Loads: {[f'{load:.2f}' for load in loads]}"
            )
